...
var UserManager = (function () {
    ...
    UserManager.prototype.initialize = function () {
        ...
        return new Promise(function (resolve, reject) {
            that.getUser(config.adminUser.username).then(function (user) {
                ...
                that.createUser(config.adminUser.username, config.adminUser.email, config.adminUser.password, 1 /* SuperAdmin */).then(function (newUser) {
                    resolve();
                }).catch(function (error) {
                    reject(error);
                });
    ...
    */
    UserManager.prototype.createUser = function (user, email, password, privilege) {
        if (privilege === void 0) { privilege = 3 /* Regular */; }
        var that = this;
        return new Promise(function (resolve, reject) {
            // Basic checks
        ...
        return new Promise(function (resolve, reject) {
            ...
            that._userCollection.findOne({ $or: target }, function (error, userEntry) {
                if (error)
                    ...
                    return reject(new Error("No user can be found with those details"));
                else
